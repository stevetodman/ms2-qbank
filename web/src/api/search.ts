...
async function requestSearch(payload: SearchPayload): Promise<SearchResponse> {
...
export async function buildSearchPayload(
  ...
  return {
    ...
    limit: filters.questionCount,
    offset: 0,
  };
}

export async function searchQuestions(filters: PracticeFilters): Promise<QuestionPayload[]> {
  const payload = await buildSearchPayload(filters);
  const response = await requestSearch(payload);
  return response.data;
}

export async function fetchFilterPreview(limit = 200): Promise<QuestionPayload[]> {
  const response = await requestSearch({ limit, offset: 0 });
  return response.data;
}
